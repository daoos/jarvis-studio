        ...
        >
         <v-progress-linear v-slot:progress color="blue" indeterminate></v-progress-linear>
          <template v-slot:items="props">
            ...
            <td><v-chip :color=props.item.statusColor text-color="white" small class="text-lowercase">{{ props.item["status"] }}</v-chip></td>
  ...
  methods: {
    ...
    async handleMounted() {
      ...
      try {
        ...
        let fetchResult = await store.dispatch("vmLauncherRuns/fetchAndAdd", {where, limit: 0});
