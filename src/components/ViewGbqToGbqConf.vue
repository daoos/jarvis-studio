  ...
  <v-container grid-list-xl fluid>
    ...
    >
      ...
      <template v-slot:items="props">
        ...
        <td>{{ props.item["configuration_dump"]["default_bq_dataset"] }}</td>
        <td>{{ props.item["nb_tasks"] }} </td>
        <td>{{ props.item["nb_dag_dependencies"] }}</td>
  ...
  computed: {
    ...
    getGbqToGbqConfFormated() {
      ...
      var dataFormated = dataArray.map(function(data, index) {
        return {
          nb_tasks: data.configuration_dump.workflow.length,
          nb_dag_dependencies: data.configuration_dump.dag_dependencies.length
        };
      });
